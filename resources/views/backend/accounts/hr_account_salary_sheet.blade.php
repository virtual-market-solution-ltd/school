                ...
                <div class="card m-b-20">
                    <div class="card-body">
                        ...
                        <table class="table table-bordered table-condensed">
                            ...
                            <tbody>
                                <form action="{{ route('accounts.hr_account_salary_sheet') }}" method="get">
                                <tr>
                                    <td>
                        ...
                        <h4 class="mt-0 m-b-30 header-title">Salary Process Month - Accounts</h4>
                        <form action="{{ route('accounts.hr_account_salary_sheet_update') }}" method="post">
                        @csrf
                        @method('put')
                        <table class="table table-striped table-bordered now-wrap  table-condensed text-center">
                            <tbody>
                                <tr>
                                    <td>From Account</td>
                                </tr>
                                <tr>
                                    <td>
                                        <select class="form-control" name="account_code" id="" required>
                                            <option value="">Choose Your Option</option>
                                            @foreach($chart_master as $row )
                        ...
                        <table class="table table-striped table-bordered now-wrap  table-condensed text-center" id="datatable-buttons">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>
                                    <input class="form-control" type="hidden" name="emp_code[]" value="{!! $row->emp_code !!}">
                                    {!! $row->emp_code !!}
                                    </td>
                                    <td>
                                        <input class="form-control" type="hidden" name="emp_account_code[]" value="{!! $row->account !!}">
                                        {!! $row->account !!}
                                    </td>
                                    <td>
                                        <input class="form-control" type="hidden" name="amount[]" value="{!! $row->amount !!}">
                                        {!! $row->amount !!}
                                    </td>
                                    <td>
                                        @if($row->status == 0) 
